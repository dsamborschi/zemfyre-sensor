...
async function ping (app) {
    ...
    const PING_ENDPOINT = app.config.telemetry.url || 'http://localhost:3000/ping'

    // const isLicensed = app.license.active()
    const isInitialised = PlatformSettings.get('setup:initialised')
    const isTelemetryEnabled = (app.config.telemetry.enabled !== false && PlatformSettings.get('telemetry:enabled'))

    if (isInitialised && isTelemetryEnabled) {
        const payload = await gather(app)
        try {
            console.log('Sending telemetry report')
            await axios.post(PING_ENDPOINT, payload, {
                responseType: 'json'
            })
            console.log('Sent telemetry report')
        } catch (err) {
            console.error('Failed to send telemetry report: %s', err)
        }
    }
}
