...
VALUES (
    'admin',
    'admin@zemfyre.local',
    '$2b$10$rGHvF8p4Yl9hH5K5p0YHXe5g9K5C5g9K5C5g9K5C5g9K5C5g9K5C5O',  -- admin123
...
-- Default MQTT superuser (password: 'mqtt_admin' - CHANGE IN PRODUCTION!)
INSERT INTO mqtt_users (username, password_hash, is_superuser, is_active)
VALUES (
    'mqtt_admin',
    '$2b$10$rGHvF8p4Yl9hH5K5p0YHXe5g9K5C5g9K5C5g9K5C5g9K5C5g9K5C5O',  -- Change this!
    true,
    true
) ON CONFLICT (username) DO NOTHING;

-- Default ACL rules
-- Allow all authenticated users to publish to their own device topics
INSERT INTO mqtt_acls (username, topic, access, priority)
VALUES 
    -- Allow devices to publish sensor data
    (NULL, 'sensor/#', 2, 10),  -- Publish to sensor/*
    (NULL, 'device/+/status', 2, 10),  -- Publish device status
    
    -- Allow devices to subscribe to commands
    (NULL, 'device/+/command/#', 1, 10),  -- Subscribe to commands
    (NULL, 'device/+/config', 1, 10),  -- Subscribe to config updates
    
    -- System topics (read-only for non-superusers)
    (NULL, 'system/#', 1, 5),
    
    -- Alert topics (read for all)
    (NULL, 'alerts/#', 1, 5)
...
-- Add authentication event types to audit_logs
COMMENT ON COLUMN audit_logs.event_type IS 'Event types: user_login, user_logout, mqtt_connect, mqtt_disconnect, mqtt_publish, mqtt_subscribe, password_change, api_key_rotation, etc.';
...
-- View: MQTT access control summary
CREATE OR REPLACE VIEW mqtt_access_summary AS
SELECT 
    mu.username,
    mu.is_superuser,
...
GROUP BY mu.username, mu.is_superuser, mu.is_active, d.uuid, d.device_name, d.is_active;
...
-- Trigger: Auto-update mqtt_users.updated_at
DROP TRIGGER IF EXISTS trigger_mqtt_users_updated_at ON mqtt_users;
CREATE TRIGGER trigger_mqtt_users_updated_at
    BEFORE UPDATE ON mqtt_users
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

-- Function: Create MQTT user when device is provisioned
CREATE OR REPLACE FUNCTION create_mqtt_user_for_device()
RETURNS TRIGGER AS $$
BEGIN
    -- Only create if mqtt_username is set
    IF NEW.mqtt_username IS NOT NULL THEN
        -- Create MQTT user (password will be set separately via API)
        INSERT INTO mqtt_users (username, password_hash, is_superuser, is_active)
        VALUES (
            NEW.mqtt_username,
            '$2b$10$invalid',  -- Placeholder - must be set via API
            false,
            NEW.is_active
        )
        ON CONFLICT (username) DO NOTHING;
        
        -- Create default ACL rules for this device
        INSERT INTO mqtt_acls (username, topic, access, priority)
        VALUES 
            (NEW.mqtt_username, 'device/' || NEW.uuid || '/#', 3, 20),  -- Full access to own topics
            (NEW.mqtt_username, 'sensor/' || NEW.uuid || '/#', 2, 20),  -- Publish sensor data
            (NEW.mqtt_username, 'system/status', 1, 10)  -- Read system status
        ON CONFLICT DO NOTHING;
    END IF;
...
DROP TRIGGER IF EXISTS trigger_create_mqtt_user_for_device ON devices;
CREATE TRIGGER trigger_create_mqtt_user_for_device
    AFTER INSERT OR UPDATE OF mqtt_username ON devices
    FOR EACH ROW
    EXECUTE FUNCTION create_mqtt_user_for_device();

COMMENT ON TABLE users IS 'Dashboard users with JWT authentication';
COMMENT ON TABLE mqtt_users IS 'MQTT broker users (mosquitto-go-auth compatible)';
COMMENT ON TABLE mqtt_acls IS 'MQTT topic access control lists (mosquitto-go-auth compatible)';
