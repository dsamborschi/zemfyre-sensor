...
module.exports = async function (_opts) {
    let mailTransport
    let exportableSettings = {}
    let EMAIL_ENABLED = config.email.enabled
    let poStartupCheck
    const isConfigured = EMAIL_ENABLED && (config.email.smtp || config.email.transport || config.email.ses)
    const mailDefaults = { from: config.email.from ? config.email.from : '"Iotistic Platform" <donotreply@iotistic.ca>' }
    ...
    if (isConfigured) {
        poStartupCheck = setInterval(() => {
            const notReady = config.email.enabled === true && EMAIL_ENABLED === false
            if (notReady) {
        ...
        if (EMAIL_ENABLED || retry) {
            ...
            if (config.email.smtp) {
                ...
                mailTransport.verify(err => {
                    if (err) {
                        console.error('Failed to verify email connection: %s', err.toString())
                        EMAIL_ENABLED = false
                    } else {
            ...
            } else if (config.email.transport) {
                mailTransport = nodemailer.createTransport(config.email.transport, mailDefaults)
                exportableSettings = { }
                console.log('Email using config provided transport')
                EMAIL_ENABLED = true
                callback && callback(null, EMAIL_ENABLED)
            } else if (config.email.ses) {
                ...
                mailTransport.verify(err => {
                    if (err) {
                        ...
                        console.log('Connected to AWS SES')
                        EMAIL_ENABLED = true
                    }
                    callback && callback(err, EMAIL_ENABLED)
                })
            } else if (!retry) {
                console.log('Email not configured - no external email will be sent')
            }
        } else {
    ...
    async function send (user, templateName, context) {
        const forgeURL = config.base_url
        ...
        if (EMAIL_ENABLED) {
            if (mailTransport) {
                mailTransport.sendMail(mail, err => {
                    if (err) {
                        log.warn(`Failed to send email: ${err.toString()}`)
                    }
                })
            }
        }
